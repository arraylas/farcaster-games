...
export default function OXOXGame() {
  ...
  function computerMove() {
    const empty = board.map((v, i) => (v === null ? i : null)).filter((i) => i !== null) as number[];
  ...
  async function shareResult() {
    if (!user) {
      alert('Please login with Farcaster to share your result.');
      return;
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
      ...
      >
        {board.map((val, idx) => (
          <button key={idx} onClick={() => handleClick(idx)} style={{ width: '40px', height: '40px', fontSize: '20px' }}>
            {val}
          </button>
        ))}
      </div>
      <div style={{ marginTop: '15px' }}>
        <button onClick={resetBoard} style={{ padding: '10px 20px' }}>
          Reset
        </button>
        <button onClick={shareResult} style={{ padding: '10px 20px', marginLeft: '5px' }}>
          Share Result
        </button>
      </div>
      <div style={{ marginTop: '15px' }}>
